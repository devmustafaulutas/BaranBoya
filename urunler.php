                    ...
                    <div class="products-dropdown-menu" aria-labelledby="dropdownToggle3">
                        ...
                        if ($kategori_id && $alt_kategori_id) {
                            $subSubcategories_query = mysqli_query(
                                $con,
                                "SELECT * FROM alt_kategoriler_alt WHERE alt_kategori_id = " . intval($alt_kategori_id)
                            );
                            if (mysqli_num_rows($subSubcategories_query) > 0) {
                                ...
                                while ($subSubcategory = mysqli_fetch_array($subSubcategories_query)) {
                                    echo '<li><a class="products-dropdown-item" href="urunler.php?kategori_id='
                                        ...
                                        . htmlspecialchars($subSubcategory['isim']) .
                                        '</a></li>';
                                }
                                echo '</ul>';
                            }
...
<section class="product-list-area">
    <div class="container">
        <div class="row product-card-row">
            <?php
            if (!$kategori_id && !$alt_kategori_id) {
                // 1) Hiç kategori seçilmemişse → tüm kategorileri göster
            ...
            elseif ($kategori_id && !$alt_kategori_id) {
                ...
                if (mysqli_num_rows($subcategories_query) > 0) {
                    ...
                    while ($subcategory = mysqli_fetch_array($subcategories_query)) {
                        ...
                        echo '<a class="product-a" href="urunler.php?kategori_id='
                            . rawurlencode(encrypt_id($kategori_id))
                            . '&alt_kategori_id=' . rawurlencode(encrypt_id($subcategory['id'])) . '">';
                    ...
                    // Alt kategori yoksa → bu kategoriye bağlı ürünleri göster
                    $products_query = mysqli_query(
                        $con,
                        "SELECT * FROM urunler WHERE kategori_id = " . intval($kategori_id)
                    );
                    if (mysqli_num_rows($products_query) > 0) {
                        while ($product = mysqli_fetch_array($products_query)) {
                            ...
                            echo '<a class="product-a" href="product_detail?urun_id='
                                . rawurlencode(encrypt_id($product['id'])) . '" class="product-card-link">';
            ...
            elseif ($alt_kategori_id && !$alt_kategori_alt_id) {
                ...
                if (mysqli_num_rows($subSubcategories_query) > 0) {
                    // Alt-alt kategoriler varsa → onları göster
                    while ($subSubcategory = mysqli_fetch_array($subSubcategories_query)) {
                        echo '<div class="col-md-4 mb-4 subcategory-card">';
                        echo '<a class="product-a" href="urunler.php?kategori_id='
                            . rawurlencode(encrypt_id($kategori_id))
