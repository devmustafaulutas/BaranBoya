            ...
            <div class="col-md-3">
                <div class="products-category-box">
                    ...
                    <div class="products-dropdown-menu" aria-labelledby="dropdownToggle1">
                        <?php if ($kategori_id): ?>
                            <?php
                            // MUTLAKA alt_kategoriler tablosunu çek
                            $subs = mysqli_query(
                                $con,
                                "SELECT * FROM alt_kategoriler WHERE kategori_id = " . intval($kategori_id)
                            );
                            ?>
                            <?php if (mysqli_num_rows($subs)): ?>
                                <ul>
                                    <?php while ($sub = mysqli_fetch_assoc($subs)): ?>
                                        <li>
                                            <a class="products-dropdown-item" href="urunler.php?kategori_id=<?= rawurlencode(encrypt_id($kategori_id)) ?>&alt_kategori_id=<?= rawurlencode(encrypt_id($sub['id'])) ?>">
                                                <?= htmlspecialchars($sub['isim']) ?>
                                            </a>
                                        </li>
                                    <?php endwhile; ?>
                                </ul>
